 ...
 * FileName: DateTools
 * Author:   zhangfan
 * Date:     2019-03-01 17:46
 * Description: 日期工具类
 * History:
 * <author>          <time>          <version>          <desc>
 * 作者姓名           修改时间           版本号              描述
 */
package cn.van.Tools;
...
public class DateTools {
    ...
    /**
     * LocalDateTime 日期转化为指定日期格式类型
     * @param localDateTime 日期
     * @param pattern 日期格式类型，如yyyy-MM-dd，yyyy年MM月dd日 HH:mm:ss
     * @return
     */
    public static String localDateTimeToString(LocalDateTime localDateTime, String pattern){
        DateTimeFormatter sdf = DateTimeFormatter.ofPattern(pattern);//日期格式
        String date_str = sdf.format(localDateTime);//日期转化为指定格式
        return date_str;
    }
     ...
     * @throws ParseException
     */
    public static LocalDateTime stringToLocalDateTime(String date_str,String pattern){
     ...
     * 日期字符串转化为指定日期格式类型
     * @param date_str 日期字符串
     * @param pattern 日期格式类型，如yyyy-MM-dd，yyyy年MM月dd日
     * @return
     * @throws ParseException
     */
    public static LocalDate stringToLocalDate(String date_str,String pattern){
        DateTimeFormatter formatter = DateTimeFormatter.ofPattern(pattern);
