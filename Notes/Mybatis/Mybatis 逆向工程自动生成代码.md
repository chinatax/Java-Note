...
> 本文主要介绍使用`Maven`插件的方式自动生成`Mybatis`相应的实体、`Mapper`、映射文件等，能应付简单的`CRUD`（`Create`/`Retrieve`/ `Update`/`Delete`）


## 一、上手使用

### 1.1 引入`MyBatis Generator`的`Maven`插件
...
> 为了便于配置表及实体名称，我把`generatorConfig.xml`中的表名和实体名称放在这里了。
...
#生成的实体类的包名
projectPath=cn.van.mybatis.generator
# 数据库表名
tableName=user_info_generator
# 实体名
modelName=UserInfoGenerator
```
