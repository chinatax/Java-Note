...
- 请求类型： `GET`
- 请求地址： [获取图片验证码url](http://localhost:8080/code/getImgCode)
- 返回结果： 
...
- 请求类型： `GET`
- 请求地址： [校验图片验证码url](http://localhost:8080/code/checkImgCode?imgCode=6pt2&imgCodeKey=26393d5c-5d56-4ac3-bf38-c2cac1f33181)

- 返回结果：验证码输入正确/验证码输入错误/图片验证码已过期，请重新获取
- 说明：前端需将请求验证码时返回的`imgCodeKey ` 字段的值以及用户数据的验证码请求到后端，进行校验；验证码有效期为五分钟，可按照实际需求配置。


[Github 示例代码](https://github.com/vanDusty/SpringBoot-Home/tree/master/springboot-demo-list/imgValidateCode)